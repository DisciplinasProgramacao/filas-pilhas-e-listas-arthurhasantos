...
public class Pedido {
	...
	public int repeticoes(Produto produto){
		// Criar um predicado que compare a descrição do produto
		Predicate<Produto> comparador = (prod -> prod.compareTo(produto) == 0);
		// Usar este predicado para retornar a repetição dos produtos no pedido
		return produtos.contarRepeticoes(comparador);
	}
